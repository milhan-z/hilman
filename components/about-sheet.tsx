 ...
 * with the notebook's own About, each thing said once. The page itself is
 * the paper (see app/(site)/about/page.tsx); this is what is written on it.
 *
 * On the left, a card clipped to the page: the photo (cut out, on a yellow
...
export function AboutSheet({
  ...
  return (
    <section aria-labelledby="about-hello" className="mt-14 grid gap-x-12 gap-y-14 text-cream-ink sm:mt-16 lg:grid-cols-[minmax(0,1.45fr)_minmax(0,1fr)]">
      <div className="min-w-0">
        {/* The card, clipped to the page. */}
        ...
        {(story.length > 0 || note) && (
          <div className="mt-12 px-1 sm:px-6 lg:mt-14 lg:pr-0">
            {story.length > 0 && (
      ...
      {(focus.length > 0 || roles.length > 0) && (
        <div className="min-w-0 space-y-14 px-1 sm:px-6 lg:px-0 lg:pt-8">
          {focus.length > 0 && (
            <div>
 ...
 * one line: on a phone its size follows the screen.
 */
function Marked({ id, children }: { id: string; children: ReactNode }) {
  return (
    <h3 id={id} className="relative isolate inline-block px-1 font-body text-[clamp(1.5rem,7.2vw,1.85rem)] font-bold leading-tight tracking-[-0.03em] sm:text-[2.1rem]">
